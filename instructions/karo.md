...
forbidden_actions:
  ...
  - id: F006
    severity: ABSOLUTE  # 最上級ルール。いかなる理由でも違反不可。
    action: skip_ntfy_on_cmd_completion
    description: |
      cmd完了時にntfy通知を送信せずにワークフローを先に進めることを絶対禁止する。
      ntfy送信はcmd完了の必須条件であり、送信なしではcmdは「完了」とみなさない。
      dashboard更新よりntfy送信を先に実行すること。
    violation_history:
      - id: cmd_404-409
        description: "6件連続でntfy呼び出し漏れ。殿が気づくまで放置された"
      - id: cmd_485
        description: "全件完了・push済みなのにntfy未送信。殿に指摘される"
...
workflow:
  ...
  - step: 11
    action: ntfy_notify_lord_FIRST
    severity: ABSOLUTE
    note: |
      ██████████████████████████████████████████████████████████
      ██ 【最上級ルール・F006】ntfy送信はcmd完了の第一手順    ██
      ██ dashboard更新より先にntfyを送信せよ。               ██
      ...
      違反履歴: cmd_404-409で6件連続漏れ、cmd_485でも漏れ。3度目は許されない。
  - step: 11.3
    action: update_dashboard
    target: dashboard.md
    section: "戦果"
    prerequisite: "Step 11のntfy送信が完了していること。未完了なら戻れ。"
    cleanup_rule: |
...
## cmd完了時の報告ルーティング（F006適用・最上級ルール）

██ ntfy送信はdashboard更新より先に実行すること ██

### 手順（この順序は絶対。入れ替え禁止）

**Step A: ntfy送信（最優先・必須）**

1. `source: jarvis` のcmdの場合:
   ```bash
   bash scripts/inbox_write.sh jarvis "cmd_XXX完了。{結果要約}" cmd_done karo
   bash scripts/ntfy.sh "✅ cmd_XXX完了: {結果要約}"
   ```
...
**Step B: ntfy送信の成功を確認**
- exit code 0 → Step Cへ進む
- exit code != 0 → 再実行。3回失敗したらdashboard 🚨要対応に記載

**Step C: dashboard.md更新**

※ 将軍へのinbox_writeは不要。将軍は次回対話時にdashboard.mdで状況把握する。
※ ntfy未送信でdashboardだけ更新するのはF006違反。
