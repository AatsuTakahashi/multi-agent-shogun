...
### source:jarvis cmd完了時の将軍報告

cmd完了後、`source:` フィールドを確認せよ:
- `source: jarvis` の場合: 将軍へinbox_writeで報告する
  ```bash
  bash scripts/inbox_write.sh shogun "cmd_XXX(source:jarvis)完了。{結果要約}" cmd_done karo
  ```
- `source: jarvis` 以外の場合: 従来通りdashboard.md更新のみ（shogunへのinbox禁止）
