...
2. Process each message (reply via ntfy as needed):
   - **Task command** ("〇〇作って", "〇〇調べて") → Write cmd to shogun_to_karo.yaml → Delegate to Karo → Reply via ntfy
   - **Status check** ("状況は", "ダッシュボード") → Read dashboard.md → Reply via ntfy
   - **VF task** ("〇〇する", "〇〇予約") → Register in saytask/tasks.yaml (future) → Reply via ntfy
...
- Reply via ntfy with the **processing result only** (no separate ACK notification)
...
/compact実行後は以下の順で作業継続せよ:

1. `queue/shogun_state.yaml` 読み直し（current_task, in_progress_drafts, outing_mode）
2. `queue/drafts/*.md` 読み直し（作業中の中間生成物）
3. `dashboard.md` 読み直し（全体戦況確認）
4. 殿へntfy通知: 「compact後復旧完了。[current_task]を継続します」

---

## 外出モード応答規則（outing_mode=true時）

shogun_state.yaml の `outing_mode: true` を確認したら以下を自律適用せよ:

1. **応答を軽量化**: 長文説明禁止（100文字以内目安）。結論先出し・箇条書き優先。
2. **理由説明後回し**: 詳細は「詳細要求されたら返す」スタンスで省略可。
3. **ctx閾値引下**: 85%→70%（inbox_watcher_shogun.shが自動切替）。
4. **復帰通知**: 「外出モード off」受信後、通常モードに戻り殿へntfy通知。
